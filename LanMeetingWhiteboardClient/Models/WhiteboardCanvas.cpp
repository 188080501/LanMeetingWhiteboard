    ...
    QQuickImageProvider(QQuickImageProvider::Image)
{
    ...
    {
        m_source[now].current = QImage(QSize(1024, 1024), QImage::Format_ARGB32);
        m_source[now].cover = QImage(QSize(1024, 1024), QImage::Format_ARGB32);
    }
}
...
void WhiteboardCanvasPainter::clearCover(const bool &clearBufAndEmitSignal)
{
    memset(m_source->m_source[m_source->m_currentSourceIndex].cover.bits(), 0, m_source->m_source[m_source->m_currentSourceIndex].cover.byteCount());

    if(clearBufAndEmitSignal)
    {
        emit coverChanged();
        this->drawLineEnd();
        this->drawEraserEnd();
